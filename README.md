...
#### Step 2: Gaining Control, Losing Simplicity

To solve the problems of both control and observation, we must now refactor our
tests to directly set and inspect the internal state of our package. This gives
us the power to create any precondition we need and to assert that side effects
actually happened.

The result is a test suite that gives us total confidence. We are no longer
blind; we can be certain that the side effects occurred exactly as we intended.
You can see the complete code for this step in the link below:

- [customer/customer_test.go](https://github.com/maniosgrivei/go-test-drivers/blob/step-02/customer/customer_test.go)

However, this newfound control comes at a steep price. Look at what our
once-simple "happy path" test has become:
...
Our clean, simple test has ballooned into a messy and fragile script. It's
cluttered with low-level assertions about every internal slice and index map in
our repository. This is no longer a test of behavior; it's a brittle check of
implementation details. This approach is not readable, not scalable, and is a
nightmare to maintain.

We have gained control, but we have lost simplicity. There must be a better way.